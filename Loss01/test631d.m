...
for k = 1:length(ds)
    ...
    L0(4,k) = getMinimal01LossByCSA(X,t);
    min3 = min(L0(1:3,k));
    if min3 > 0
        L0(5,k) = round(100*(min3 - L0(4,k))/min3);
    end
    fprintf('***0NoiseLosses [BPM, SVM, LR, CSA] = %s. Impr: %d%%\n', ...
    ...
    L1(4,k) = getMinimal01LossByCSA(X,t);
    min3 = min(L1(1:3,k));
    if min3 > 0
        L1(5,k) = round(100*(min3 - L1(4,k))/min3);
    end
    fprintf('***1NoiseLosses [BPM, SVM, LR, CSA] = %s. Impr: %d%%\n', ...
